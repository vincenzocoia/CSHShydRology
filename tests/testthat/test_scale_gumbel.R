...
test_that("gumbelRP_trans returns a fresh transform object", {
  tr <- gumbelRP_trans()
  expect_s3_class(tr, "transform")
  expect_identical(tr$name, "gumbelRP")
  # built on demand, not stored: two calls give equal but distinct objects
  expect_equal(gumbelRP_trans()$transform(10), tr$transform(10))
})

test_that("gumbelAEP_trans returns a fresh transform object", {
  tr <- gumbelAEP_trans()
  expect_s3_class(tr, "transform")
  expect_identical(tr$name, "gumbelAEP")
})
...
test_that("return period and AEP are consistent with one another", {
  aep <- c(0.5, 0.2, 0.01)
  expect_equal(gumbelAEP_trans()$transform(aep),
               gumbelRP_trans()$transform(1 / aep))
})

test_that("the scale functions build a usable ggplot layer", {
  df <- data.frame(aep = 1:20 / 21, y = sort(stats::rexp(20), decreasing = TRUE))
  for (sc in list(scale_x_gumbelAEP(), scale_y_gumbelAEP())) {
...
test_that("arguments are passed through to the underlying ggplot2 scale", {
  sc <- scale_x_gumbelRP("Return Period")
