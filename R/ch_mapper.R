#'  ch_mapper
#'  
#'  @description Generate a mp for a defines area.  Options to plot station locations, magnitudes,
#'  trends etc.  Watershed boundaries and add user defined labels.
#'  
#'  @param map_data a list produced by the function ch_get_mapper_data
#'  @param locations a dataframe with longitude, latitude. a third column may contain indexes of symbol types.
#'    If the third column is omitted all symbols are plotted using the same symbol and colour. 
...
#'  @param 
#'  @param 
#'  @param basins a list with basin shapefiles
#'  @param b_border colour for watershed boundaries: default is "darkred"
#'  @param b_lwd width of watershed boundary
#'  @param
...
#'  @param
#'  @param x_labels a dataframe with seven columns (long, lat, pos, cex, font, col, text). 
#'  Each row provides details for a single label : default is NA
#'  
...
##########################################################
ch_mapper <- function(map_data, 
                      locations = NA, lo_pch = 19, lo_col= "black", lo_cex = 1.0, lo_text = "Station",
                      basins = NA, b_border = "darkred", b_lwd = 2.,
                    
                      trends = NA, tr_pch =c(25, 20, 24), 
                      ...
                      var = NA, vr_pch = 22, vr_cex = 2.0, vr_text = NA, vr_range = c(0,1),
                      ...
                      rivers = TRUE, boundaries = TRUE, plabels = TRUE, pl_cex = 1.0,
                      legend = FALSE, le_text = NA,
                      x_labels = NA)
                            {
  ...
  if(!is.na(basins)){
    ...
    for(kb in 1: length(basins)){
       
      n_basin <-st_transform(basins[[kb]], CRS(map_proj))
     
    plot(n_basin,  border=b_border,lwd = b_lwd, col=NA, add=TRUE)
    }
  }
  ...
  if(rivers){
  rline <- st_crop(rivers10, xmin=maplong[1],xmax=maplong[2],ymin=maplat[1],ymax=maplat[2])
  rivers_1 <-st_transform(rline, CRS(map_proj))
  plot(rivers_1,  col="blue", add=TRUE)
  }
  
  if(boundaries){
  pline <- st_crop(plines10, xmin=maplong[1],xmax=maplong[2],ymin=maplat[1],ymax=maplat[2])
  plines_1 <- st_transform(pline, CRS(map_proj))
  ...
  if(plabels){
   ...
   ### trim list of provincial labels to only those inside the map box
     ...
     map_label4 <- map_label3[map_label3$llat <= maplat[2],]
      ...
      st_as_sf(coords = c("llong","llat"))%>%
      `st_crs<-`(4326)
    

    pt_labelsa <-st_transform(map_labels, map_proj)
...
for (kk in 1:length(pt_labelsa$lpos)){
  ...
  if(!is.na(locations)){                      
    ...
    pt_sf <- data.frame(mlong, mlat, mcode) %>%
      ...
      # reproject 
      pt_sfa <-st_transform(pt_sf, map_proj)
     ...
     ################################################# plot points 
      ...
      if(legend){
        ...
        legend("topright", lo_text, pch = lo_pch,  inset=0.06, col = lo_col, pt.bg = lo_col, 
               cex = lo_cex, bg="white", title = "Locations")
      }
    }
...
################################################# plot points 
  ...
  tr_lcol <- c(tr_col[3], tr_col[3], tr_col[2], tr_col[1], tr_col[1])
    legend("topright", tr_ltext, pch = tr_lpch, cex = 0.60, inset=0.06, col = tr_lcol, pt.bg = tr_lcol, 
...
##################################################################### plot variable scaled colour
  ...
  if(legend){
    ...
    legend("topright", legend=lx, pch = 22,  inset=0.06, pt.bg = ch_color_gradient(lx, vr_colors), cex=0.8,  
  ...
  if(!is.na(sc_var)){
    ...
    if(legend){
      ...
      legend("topright", legend=lx, pch = sc_pch,  inset=0.06, pt.bg = sc_color, 
  ...
  if(!is.na(x_labels)){
    ...
    x_labelsa <-st_transform(xmap_labels, map_proj)
